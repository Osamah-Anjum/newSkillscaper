...
export default function Footer() {
  return (
    <footer className="py-12 section-divider bg-[#000001] border-t border-[#FFA400]/8">
      <div className="px-6 sm:px-12 lg:px-20 xl:px-28">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-10 text-sm">
            {cols.map(col => (
              <motion.div key={col.heading} variants={fadeUp}>
                <p className="text-xs tracking-widest uppercase mb-4 text-[#FFA400]/40">{col.heading}</p>
                <ul className="flex flex-col gap-2.5">
                  {col.links.map(l => (
                    <li key={l}>
                      <a href="#" className="text-pearl/35 hover:text-[#FFA400] transition-colors duration-200">{l}</a>
                    </li>
                  ))}
                </ul>
              </motion.div>
        ...
        <motion.div
          ...
          className="mt-12 pt-6 border-t border-[#FFA400]/6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-pearl/20"
        >
